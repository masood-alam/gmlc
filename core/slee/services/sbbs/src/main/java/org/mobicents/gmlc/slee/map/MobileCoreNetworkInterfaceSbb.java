...
//  private final InjectedValue<GmlcPropertiesManagementMBean> gmlcProperties = new InjectedValue<GmlcPropertiesManagementMBean>();
  ...
  public void setSbbContext(SbbContext sbbContext) {
    ...
    try {
      ...
      this.gmlcPropertiesManagement = GmlcPropertiesManagement.getInstance();

      if (this.gmlcPropertiesManagement == null) {

	this.gmlcPropertiesManagement = GmlcPropertiesManagement.getInstance("GmlcPropertiesManagement");
    
      	MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();
	
	ObjectName mObjectName = new ObjectName( "org.mobicents.gmlc:name=GmlcPropertiesManagement");
	ObjectInstance objectInstance = mbs.getObjectInstance(mObjectName);
	String name = objectInstance.getClassName();
 
	//GmlcPropertiesManagement g = (GmlcPropertiesManagement)objectInstance;


//	MBeanInfo info = mbs.getMBeanInfo(mObjectName);
...
//	}
	try {
	String value = (String)mbs.getAttribute(mObjectName, "GmlcGt");
	logger.info("GmlcGt = " + value);
	
	this.gmlcPropertiesManagement.setGmlcGt(value);

	int ssn = (int)mbs.getAttribute(mObjectName, "GmlcSsn");
	logger.info("GmlcSsn = " + ssn);
	
	this.gmlcPropertiesManagement.setGmlcSsn(ssn);

	ssn = (int)mbs.getAttribute(mObjectName, "HlrSsn");
	logger.info("HlrSsn =" + ssn);
	
	this.gmlcPropertiesManagement.setHlrSsn(ssn);

	} catch (Exception e) {
	logger.info("some error in setGmlcGt()");
	}
       }
//	ObjectInstance objectInstance = mbs.getObjectInstance(mObjectName);
//	String name = objectInstance.getClassName();
...
//	Object value = mbs.invoke(mObjectName, "getGmlcGT", null, null);
	//logger.info(value.toString());
	//logger.info(attrInfo.toString());

      logger.info("GmlcPropertiesManagement.getinstance()=" + GmlcPropertiesManagement.getInstance());
      logger.info("Successfully set SBB Context");
    } catch (Exception ne) {
  ...
  private void getSingleMSISDNLocation(String requestingMSISDN) {
	...
	this.logger.info("GmlcSccpAddress()=" + this.getGmlcSccpAddress());
	this.logger.info("getHlrSCCPAddress() = " + this.getHlrSCCPAddress(requestingMSISDN) );
  ...
  protected SccpAddress getGmlcSccpAddress() {
    ...
    if (this.gmlcSCCPAddress == null) {
      ...
      if (gmlcPropertiesManagement == null) {
	  this.logger.info("instance = " + GmlcManagement.getInstance() );
	}
      this.logger.info("gmlcPropertiesManagement=" + this.gmlcPropertiesManagement);
...
//		return new SccpAddressImpl(RoutingIndicator.ROUTING_BASED_ON_GLOBAL_TITLE, gt, 0, gmlcPropertiesManagement.getHlrSsn());
  ...
  private void handleLocationResponse(MLPResponse.MLPResultType mlpResultType, ATIResponse atiResponse, String mlpClientErrorMessage) {
    ...
    switch (request.type) {
	  ...
	  jsonResponse.put("mcc", atiResponse.mcc);
	  jsonResponse.put("mnc", atiResponse.mnc);
	  jsonResponse.put("lac", atiResponse.lac);
	  jsonResponse.put("cellid", atiResponse.cell);
	  jsonResponse.put("aol", atiResponse.aol);
	  jsonResponse.put("state", atiResponse.subscriberState);
		/*
		...
		*/
 	 this.sendHTTPResult(httpServletResponse.SC_OK, jsonResponse.toString());
